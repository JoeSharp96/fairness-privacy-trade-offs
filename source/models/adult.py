...
class Adult(Net):
    ...
    def test(self, testloader, device, globaltest=False):
        ...
        if globaltest:
            ...
            x_sensitive_feature = torch.cat(x_sensitive_feature, dim=0)
...
def skew_and_split(sensitive_feature: str, sensitive_value: str, _skew: float = 0.3, seed: int = 42, global_train: bool = False):
    def skew(dataset_dict):
        ...
        if global_train:
            ...
            minority_dataset = dataset.filter(lambda x: str(x[sensitive_feature]).lower().strip() == sensitive_value)
            majority_dataset = dataset.filter(lambda x: str(x[sensitive_feature]).lower().strip() != sensitive_value)
...
def load_data(partition_id: int, num_partitions: int, batch_size: int, alpha: float, sensitive_feature: str, sensitive_value: str, skew: int, seed: int):
...
def save_partitions(sensitive_feature: str, sensitive_value: str):
    global fds
    data = []
    for i in range(5):
        dataset = fds.load_partition(i, "train").with_format("pandas")[:]
        df_min = dataset[dataset[sensitive_feature].str.lower() == sensitive_value]
        df_maj = dataset[dataset[sensitive_feature].str.lower() != sensitive_value]
        row = [len(df_min),len(df_maj)]
        data.append(row)
    df = pd.DataFrame(data=data, columns=["sensitive","non-sensitive"])
